...
import com.ehpessoa.service.conf.SharePointProperties;
...
public class SharePointServiceCached {
	...
	public String receiveSecurityToken(Long executionDateTime) throws TransformerException, URISyntaxException {
		...
		String envelope = buildSecurityTokenRequestEnvelope();
		System.out.println("Envelope: " + envelope);
		
		RequestEntity<String> requestEntity = new RequestEntity<>(envelope, HttpMethod.POST,
				new URI(sharePointProperties.getEndpointToken() + "/extSTS.srf"));

		System.out.println("Token Type: " + requestEntity.getType().toString() + " - URL: " + requestEntity.getUrl());
		System.out.println("Token Request: " + requestEntity.getBody());

		ResponseEntity<String> responseEntity = restTemplate.exchange(requestEntity, String.class);

		System.out.println("Token Response: " + responseEntity.getBody());
	...
	private String buildSecurityTokenRequestEnvelope() {		
		...
		String envelopeTemplate = "<s:Envelope xmlns:s=\"http://www.w3.org/2003/05/soap-envelope\" "
				...
				+ "<o:UsernameToken>  "
				+ "<o:Username>" + sharePointProperties.getUsername() + "</o:Username>  "
				+ "<o:Password>" + sharePointProperties.getPassword() + "</o:Password> "
				+ "</o:UsernameToken>  "
				+ "</o:Security> "
				+ "</s:Header>"
				+ "<s:Body>"
				...
				+ "<a:EndpointReference>"
				+ "<a:Address>" + sharePointProperties.getEndpointDomain() + "</a:Address></a:EndpointReference>"
				+ "</wsp:AppliesTo>"
