...
public class SharePointService {
	...
	public String performHttpRequest(HttpMethod method, String path) throws Exception {
		...
		RequestEntity<String> requestEntity = new RequestEntity<>(headers, method,
				new URI(serviceCached.completeURL(path)));
	...
	public String attachFile(String path, byte[] file) throws Exception {
		...
		RequestEntity<byte[]> requestEntity = new RequestEntity<>(file, headers, HttpMethod.POST,
				new URI(serviceCached.completeURL(path)));
		ResponseEntity<String> responseEntity = restTemplate.exchange(requestEntity, String.class);
		String responseBody = responseEntity.getBody();
		log.debug(responseBody);
